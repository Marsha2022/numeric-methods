...
# ---------- 1. Метод вращения Якоби (оставлен с прежним выводом) ----------
...
# ---------- 2. QR-алгоритм (с выводом, как в примере) ----------
# Реализация QR-разложения методом Хаусхолдера
...
def qr_algorithm_symmetric(A, epsilon=1e-4, max_iter=1000):
    ...
    Вывод организован как в приведённом примере (итерации, проверки).
    """
    n = A.shape[0]
    ...
    print(f"Критерий сходимости: диагональная форма (все внедиаг < ε)")

    for iteration in range(max_iter):
        Q, R = householder_qr(A_k)
        A_next = R @ Q

        # Для симметричной матрицы: проверяем все внедиагональные элементы
...
# Извлечение собственных значений (диагональ)
